...
def predictcatption(image_path , model_version = 4):
      ...
      enc.load_weights(os.path.join(curr_dir,'ImageCap/models/encoder'+str(model_version)+'/'))
      dec.load_weights(os.path.join(curr_dir,'ImageCap/models/decoder'+str(model_version)+'/'))

      print("model Version : " , model_version)

      result, attention_plot = evaluate(image_path , enc, dec)
      bl = True 
      while(bl):
        if(len(result)<=20):
          bl =False
      ...
      result_path = plot_attention(image_path, result, attention_plot)
